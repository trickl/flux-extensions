...
public class RobustWebSocketFluxClient {
  ...
  @Builder.Default private Function<FluxSink<StompFrame>, Duration> doConnect = 
      sink -> Duration.ZERO;
  ...
  protected SharedStreamContext createSharedStreamContext(BaseStreamContext context,
    ...
    Flux<StompFrame> sharedStream = context.getStream()
        ...
        .doOnSubscribe(sub -> {
          resubscribeAll(context.getResponseContext().getStreamRequestProcessor().getSink());

          doAfterSharedSubscribe.accept(context.getResponseContext()
              .getStreamRequestProcessor().getSink());
        })
        .doOnSubscribe(sub -> {
        ...
        .retryWhen(errorFlux -> errorFlux.flatMap(error -> {
  ...
  protected Flux<SharedStreamContext> getSharedStreamContext(
    ...
    Flux<StompFrame> base = Flux.defer(() -> stompFluxClient.get(sendWithResponse))
          ...
          .share()
          .doOnSubscribe(sub -> 
            log.info("** SUBSCRIBING **" + sub.toString())
          )
          .doOnCancel(() -> 
            log.info("** CANCEL  **")
          )
    ...
    Flux<StompFrame> heartbeats = Flux.from(
        ...
        .<StompFrame>switchMap(
          ...
          context.getStreamRequestProcessor().getSink()))
        .log("heartbeats");
    ...
    ExpectedResponseTimeoutFactory<StompFrame> heartbeatExpectationFactory = 
        ...
        .build();
    Publisher<StompFrame> heartbeatExpectation = heartbeatExpectationFactory.apply(
        context.getHeartbeatExpectationProcessor(), base);

    Flux<StompConnectedFrame> heartbeatExpectations = 
         Flux.from(heartbeatExpectation).mergeWith(heartbeats).cast(StompConnectedFrame.class);
    ...
    Publisher<StompFrame> requireReceiptExpectation = receiptExpectationFactory.apply(
        context.getReceiptExpectationProcessor(), base);

    ExpectedResponseTimeoutFactory<StompConnectedFrame> connectionExpectationFactory = 
        ExpectedResponseTimeoutFactory.<StompConnectedFrame>builder()
        .isRecurring(false)
        .isResponse(value -> true)
        .timeoutExceptionMapper((error, period) -> 
            new ConnectionTimeoutException("No connection within " + period, error))
        .build();
    Publisher<StompConnectedFrame> connectionExpectation = connectionExpectationFactory.apply(
    ...
    Flux<StompConnectedFrame> connectedWithExpectations = connected
        ...
        .mergeWith(Flux.from(requireReceiptExpectation).cast(StompConnectedFrame.class))
        .log("sharedConnectedWithExpectations", Level.INFO)
        .share()
        .log("connectedWithExpectations", Level.INFO);

    Flux<StompFrame> connectedBase = base.mergeWith(
        connectedWithExpectations.filter(value -> false))
        .log("connectedBase", Level.INFO);

    return connectedWithExpectations.flatMap(connectedFrame -> 
       Mono.fromSupplier(() -> 
          createBaseStreamContext(context, connectedBase, connectedFrame))
        .flatMapMany(
          baseStreamContext -> {
  ...
  public <T> Flux<T> subscribe(
    ...
    Consumer<FluxSink<StompFrame>> doAfterSharedStreamSubscribe = sink -> {
      log.info("Subscribing to " + destination);
      subscriptionDestinationIdMap.computeIfAbsent(destination, 
          d -> subscribeDestination(d, sink));      
    };
  ...
  protected <T> Flux<T> subscribe(SharedStreamContext context, 
    ...
    Flux<StompMessageFrame> messageFrameFlux =
        ...
        .doFinally(signal -> unsubscribe(destination, 
          context.getBaseStreamContext().getResponseContext()
          .getStreamRequestProcessor().getSink()))
